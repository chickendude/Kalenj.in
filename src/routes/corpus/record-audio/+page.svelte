<script lang="ts">
	...
	function navigateTo(nextQuery: string, nextLang: CorpusSearchLanguage) {
		...
		goto(`/corpus/record-audio${search ? `?${search}` : ''}`, {
			keepFocus: true,
			noScroll: true,
			replaceState: true
		});
...
<svelte:head>
	<title>Record audio · Corpus</title>
</svelte:head>

<section>
	<div class="page-head">
		<div>
			<div class="page-kicker">Corpus</div>
			<h1>Record audio</h1>
			<p>Record sentences in bulk. Pause between sentences to advance to the next sentence.</p>
		</div>
	</div>
